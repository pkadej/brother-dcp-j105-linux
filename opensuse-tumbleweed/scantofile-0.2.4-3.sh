...
batchstart=1
batchincrement=2
echo debug: scanimage --device-name="$device" --resolution $resolution --format=tiff --mode "$mode" --batch-start=$batchstart --batch-increment=$batchincrement --batch="$tmp_dir/tmp-image%02d"
...
scanimage --device-name="brother4:net1;dev0" --resolution 300 --format=tiff --mode "24bit Color" --batch-start=1 --batch-increment=1 --batch="$tmp_dir/tmp-image%02d"
echo DOOON!!
